...
func processJson(routingKey string, indata []byte) (string, error) {
	...
	msgs, err := fwd.ProcessJSONMessage(msg, routingKey)
	if err != nil {
		return "", err
	}
...
func processProtobuf(routingKey string, indata []byte) (string, error) {
	msg, err := fwd.ProcessProtobufMessage(routingKey, indata)
	if err != nil {
		return "", err
	}
	msg["cb_server"] = "cbserver"
...
func BenchmarkProtobufEventProcessing(b *testing.B) {
	fn := path.Join("./raw_data/protobuf/ingress.event.process/0.protobuf")
	fp, _ := os.Open(fn)
...
func BenchmarkJsonEventProcessing(b *testing.B) {
	fn := path.Join("./raw_data/json/watchlist.hit.process/0.json")
	fp, _ := os.Open(fn)
	d, _ := ioutil.ReadAll(fp)
...
func TestEventProcessing(t *testing.T) {
	...
	for _, format := range formats {
		pathname := path.Join("./raw_data", format.formatType)
		...
		for _, info := range infos {
			...
			os.MkdirAll(path.Join("./go_output", format.formatType, routingKey), 0755)
			...
			for _, fn := range files {
				...
				outfp, err := os.Create(path.Join("./go_output", format.formatType, routingKey, fn.Name()))
				if err != nil {
