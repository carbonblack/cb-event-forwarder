...
val artifactoryBaseUrl: String = System.getenv("ARTIFACTORY_BASE_URL") ?: ""
...
val buildEventForwarderDockerImageTask = tasks.register<Exec>("buildEventForwarderDockerImage") {
    ...
    args("build", "./docker/", "--tag",
        "cb-pub-docker-dev-local2.$artifactoryBaseUrl/cb-pub-docker-dev-local2/cb/event-forwarder:${versionDetails().branchName}-$currentVersion")
...
val dockerLogin = tasks.register<Exec>("dockerLogin") {
    ...
    executable("docker")
    args("login", "-u", user, "-p", pw, artifactoryBaseUrl)
}
...
val publishEventForwarderDockerImageTask = tasks.register<Exec>("publishEventForwarderDockerImageTask") {
    ...
    dependsOn(buildEventForwarderDockerImageTask)
    executable("docker")
    args("push", "cb-pub-docker-dev-local2.$artifactoryBaseUrl/cb-pub-docker-dev-local2/cb/event-forwarder:${versionDetails().branchName}-$currentVersion")
}
