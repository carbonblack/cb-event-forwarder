...
func ProcessProtobufMessage(routingKey string, body []byte, headers amqp.Table) (map[string]interface{}, error) {
	...
	if eventMsg {
		...
		if cbMessage.Childproc != nil {
			outmsg["parent_process_guid"] = processGUID
			outmsg["parent_pid"] = inmsg.OriginalMessage.Header.GetProcessPid()
		} else {
...
func WriteChildprocMessage(message *ConvertedCbMessage, kv map[string]interface{}) {
	...
	if om.Childproc.Pid != nil && om.Childproc.CreateTime != nil && om.Env != nil &&
		...
		kv["process_guid"] = MakeGUID(SensorId, pid32, createTime)
	} else {
		kv["process_guid"] = om.Childproc.GetChildGuid()
	}

	kv["pid"] = om.Childproc.GetPid()
	...
	if om.Childproc.Suppressed != nil &&
		...
		kv["suppressed"] = true
		kv["command_line"] = GetUnicodeFromUTF8(om.Childproc.GetCommandline())
