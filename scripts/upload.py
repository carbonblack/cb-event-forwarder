...
Auth: ACCESS_ID + ACCESS_TOKEN (or ARTIFACTORY_USER + ARTIFACTORY_API_KEY).
"""
...
ARTIFACTORY_PUB_URL = "https://" + os.environ.get("ARTIFACTORY_BASE_URL", "") + "/artifactory"
...
def _property_headers() -> Dict[str, str]:
    token = os.environ.get("ACCESS_TOKEN") or os.environ.get("ARTIFACTORY_API_KEY", "")
...
class UploadRunner:
    ...
    def __init__(self, user: Optional[str], api_key: Optional[str], os_type: Optional[str]) -> None:
        self.username = user or os.environ.get("ACCESS_ID") or os.environ.get("ARTIFACTORY_USER")
        self.api_key = str(
            api_key or os.environ.get("ACCESS_TOKEN") or os.environ.get("ARTIFACTORY_API_KEY") or ""
        )
